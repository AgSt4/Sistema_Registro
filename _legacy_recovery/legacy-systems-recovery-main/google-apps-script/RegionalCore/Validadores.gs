...
/* ============================================================
   📲 WHATSAPP ENGINE (Optimizado y Dinámico)
   ============================================================ */
...
function inyectarFormulaWhatsApp(sheet, headers) {
  ...
  const colApe = headers['Apellido1'] ? columnToLetter(headers['Apellido1']) : null;
  ...
  if (!colNom || !colTel || !colRol) return; // Faltan datos vitales

  // 2. Construir Fórmula
  // Usamos referencias relativas a la fila 2 (Ej: C2, D2)
  // Lógica simplificada pero potente.
  
  const formula = `={"WhatsApp"; ARRAYFORMULA(IF(${colTel}2:${colTel}=""; ""; ` +
    `HYPERLINK("https://wa.me/56" & SUBSTITUTE(SUBSTITUTE(${colTel}2:${colTel}; "+"; ""); " "; "") & "?text=" & ENCODEURL(` +
      `IFS(` +
        // Caso A: Escolares
        `${colRol}2:${colRol}="Escolares"; "Hola " & ${colNom}2:${colNom} & ", te escribo desde Ideapaís 😊"; ` +
        // Caso B: Universitarios
        `${colRol}2:${colRol}="Universitarios"; "Hola " & ${colNom}2:${colNom} & ", ¿cómo va el semestre? Te escribo desde Ideapaís 👋"; ` +
        // Caso C: Profesionales (Con distinción de sexo)
        `${colRol}2:${colRol}="Profesionales"; IF(${colSex}2:${colSex}="Femenino"; "Estimada "; "Estimado ") & ${colNom}2:${colNom} & ", espero que estés muy bien. Te contacto desde Ideapaís."; ` +
        // Caso D: Profesores
        `${colRol}2:${colRol}="Profesores"; IF(${colSex}2:${colSex}="Femenino"; "Estimada "; "Estimado ") & ${colNom}2:${colNom} & ", un gusto saludarle. Le escribo desde Ideapaís."; ` +
        // Caso E: Staff/Ideapaís
  ...
  // 3. Inyectar en Fila 1 (Cabecera + ArrayFormula)
